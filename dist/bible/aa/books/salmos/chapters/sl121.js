"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.sl121 = void 0;
exports.sl121 = [
    {
        number: 1,
        text: "Elevo os meus olhos para os montes; de onde me vem o socorro?",
    },
    {
        number: 2,
        text: "O meu socorro vem do Senhor, que fez os céus e a terra.",
    },
    {
        number: 3,
        text: "Não deixará vacilar o teu pé; aquele que te guarda não dormitará.",
    },
    {
        number: 4,
        text: "Eis que não dormitará nem dormirá aquele que guarda a Israel.",
    },
    {
        number: 5,
        text: "O Senhor é quem te guarda; o Senhor é a tua sombra à tua mão direita.",
    },
    { number: 6, text: "De dia o sol não te ferirá, nem a lua de noite." },
    {
        number: 7,
        text: "O Senhor te guardará de todo o mal; ele guardará a tua vida.",
    },
    {
        number: 8,
        text: "O Senhor guardará a tua saída e a tua entrada, desde agora e para sempre.",
    },
];
