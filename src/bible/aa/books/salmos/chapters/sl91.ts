export const sl91 = [
  {
    number: 1,
    text: "Aquele que habita no esconderijo do Altíssimo, à sombra do Todo-Poderoso descansará.",
  },
  {
    number: 2,
    text: "Direi do Senhor: Ele é o meu refúgio e a minha fortaleza, o meu Deus, em quem confio.",
  },
  {
    number: 3,
    text: "Porque ele te livra do laço do passarinho, e da peste perniciosa.",
  },
  {
    number: 4,
    text: "Ele te cobre com as suas penas, e debaixo das suas asas encontras refúgio; a sua verdade é escudo e broquel.",
  },
  {
    number: 5,
    text: "Não temerás os terrores da noite, nem a seta que voe de dia,",
  },
  {
    number: 6,
    text: "nem peste que anda na escuridão, nem mortandade que assole ao meio-dia.",
  },
  {
    number: 7,
    text: "Mil poderão cair ao teu lado, e dez mil à tua direita; mas tu não serás atingido.",
  },
  {
    number: 8,
    text: "Somente com os teus olhos contemplarás, e verás a recompensa dos ímpios.",
  },
  {
    number: 9,
    text: "Porquanto fizeste do Senhor o teu refúgio, e do Altíssimo a tua habitação,",
  },
  {
    number: 10,
    text: "nenhum mal te sucederá, nem praga alguma chegará à tua tenda.",
  },
  {
    number: 11,
    text: "Porque aos seus anjos dará ordem a teu respeito, para te guardarem em todos os teus caminhos.",
  },
  {
    number: 12,
    text: "Eles te susterão nas suas mãos, para que não tropeces em alguma pedra.",
  },
  {
    number: 13,
    text: "Pisarás o leão e a áspide; calcarás aos pés o filho do leão e a serpente.",
  },
  {
    number: 14,
    text: "Pois que tanto me amou, eu o livrarei; pô-lo-ei num alto retiro, porque ele conhece o meu nome.",
  },
  {
    number: 15,
    text: "Quando ele me invocar, eu lhe responderei; estarei com ele na angústia, livrá-lo-ei, e o honrarei.",
  },
  {
    number: 16,
    text: "Com longura de dias fartá-lo-ei, e lhe mostrarei a minha salvação.",
  },
];
